...
class GreedyAgent(Agent):
    ...
    def step(self, shape):
        ...
        print(f'found {len(feasible_positions)} feasible positions for shape {shape.id}')
        ...
        best_score = np.argmax(avg_scores)
        best_pos = feasible_positions[best_score]
        print(f'adding shape {shape.id} at position {best_pos[1], best_pos[2], best_pos[3]} with a score of {avg_scores[best_score]}')
